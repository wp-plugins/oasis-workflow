...
class FCWorkflowActions
{
	...
	static function include_files($page, $css=null, $js=null)
	{
		...
		//echo "<script type='text/javascript' src='" . OASISWF_URL . "js/lib/calendar/jquery.ui.datepicker.js' ></script>";
	...
	{
		if( $_POST["save_action"] == "submit_post_to_workflow" ){
			FCProcessFlow::submit_post_to_workflow() ;
		}
		
		if( $_POST["hi_process_info"] ){
	...
	{
		global $wpdb;
		$histories = FCProcessFlow::get_action( array( "post_id" => $postid ) ) ;
		if( $histories )
		{
