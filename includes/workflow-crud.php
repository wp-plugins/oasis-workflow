...
class FCWorkflowCRUD extends FCWorkflowBase
{
	static function save()
	{
		...
		if($_POST["deleted_step_ids"]){
			...
			{
				$wpdb->get_results( "DELETE FROM fc_workflow_steps WHERE ID = " . $deleted_steps[$i] ) ;		
			}
		}
	...
	static function as_save()
	{
		...
		if($wf){
			...
			$newWfId = FCWorkflowCRUD::insert_to_table("fc_workflows", $data );
	...
	{
		global $wpdb ;
		$newStepId = FCWorkflowCRUD::same_as_save( "fc_workflow_steps", $stepid ) ;
		if( $newStepId ){
			$wpdb->update("fc_workflow_steps", array( "workflow_id" => $newWfId ), array( "ID" => $newStepId ) ) ;
			return $newStepId ;
		}else{
			return false;
		}
						...
						<tr height="50px">
							...
							if($oasiswf_path){								
								foreach ($oasiswf_path as $k => $v) {							
									$str .= '<td width="110px;">
												<input type="radio" id="path-opt-' . $v[1] . '" name="path-opt" value="' . $v[1] . '" ' . $chk . '/> ' . $v[0] . '  
											</td>';									
								}								
							}							
				$str .=	'</tr>
	...
	static function create_new_workflow()
	{
		...
		$newid = FCWorkflowCRUD::insert_to_table( "fc_workflows", $data ) ;
		echo $newid ;
		exit();
	}
	...
	static function wokflow_data_update($param)
	{
		...
		if( $result ){			
			...
			if( $wf_info->steps->$param["stepgpid"] ){			
				...
				$result = $wpdb->update( 'fc_workflows', array('wf_info' => $saveData), array('ID' =>  $param["wpid"]) );
			}
		}
	}
	...
	static function workflow_step_save()
	{
		...
		$result = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM fc_workflow_steps WHERE ID = '" . $_POST["stepid"] . "'" ) );
		if(	$result ) {
			$result = $wpdb->update(
						'fc_workflow_steps',
						array(
			...
			$insert_row = $wpdb->get_row("SELECT max(ID) as maxid FROM fc_workflow_steps");		
	...
	static function get_previous_workflow_version($wfid, $field_name=null)
	{
		...
		$sql = "SELECT * FROM fc_workflows WHERE version=" .  ( $workflow->version - 1 ) . " && (parent_id = " . $parent_id . " || ID = " . $parent_id .  " )" ;
