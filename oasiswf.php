...
define( 'OASISWF_VERSION' , '1.0' );
define( 'OASISWF_DB_VERSION','1.0');
...
class FCInitialization
{
	...
	static function run_on_activation()
	{
		...
		{
			FCInitialization::upgrade_database();
		}
	...
	{
		print_r("nothing to upgrade yet"); die();
	}
	...
	function install_database()
	{
		...
		$table_sql_arr= FCInitialization::get_table_sql_arr();
		foreach($table_sql_arr as $sql)
		{
			dbDelta($sql);
		}
	...
	function get_table_sql_arr()
	{
		$sql_arr=array(
	    'CREATE TABLE IF NOT EXISTS `fc_workflows` (
    	...
    	,'CREATE TABLE IF NOT EXISTS `fc_workflow_steps` (
		      `ID` int(11) NOT NULL AUTO_INCREMENT,
    	...
    	,'CREATE TABLE IF NOT EXISTS `fc_emails` (
		    `ID` int(11) NOT NULL AUTO_INCREMENT,
		    `subject` mediumtext,
		    `message` mediumtext,
		    `from_user` int(11),
      	...
      	,'CREATE TABLE IF NOT EXISTS `fc_action_history` (
			    `ID` int(11) NOT NULL AUTO_INCREMENT,
	...
	static function add_js_file($jsfile_arr)
	{
		if(is_array($jsfile_arr))
		{
			...
			{
				$myJsDir = WP_PLUGIN_DI . '/oasis-workflow/js/'.$jsfile.'.js';
